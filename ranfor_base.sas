%macro ranfor_base(_infile_data_,list_var_file,store_filename,_target_type_,ntrees,maxnvar,
_ouput_internal_score_,_seed_,_trainfrRF_=0.6,_maxdepth_=50,_leaffrac_=0.0001,_alpha_=0.05,
_targety_=Y,_targetn_=N)
...
5) ntrees: number of trees in the (random) forest.
6) maxnvar: number of variables to be selected randomly at node splitting.
7) _ouput_internal_score_
8) _seed_: seed of the random number generator.
9) _trainfrRF_: in the random forest algorithm the fraction of the  input data set that is used to grow a tree (default=0.6)
	input data set = train data set created via "trainvalidation" and "balance" choices. 
10) _maxdepth_: the maximum number of nodes in each tree of the forest is 2^_maxdepth_ (provided that there are enough observation to populate all the leaves)
11) _leaffrac_: determines the minimum number of records a leaf must have for a splitting to occurr as the fraction of the number of records available for the growth of each tree 
...
12) _alpha_: specifies a threshold p-value for the significance level of a test of association of a candidate variable with the target. 
...
%put Executing: ranfor_base with data &_infile_data_;
