...
strata=target,whichlib=same) / store source des="";
...
5) _distribution_: type of possible distribution for the target variable.
	Valid values for the distribution parameter are POISSON, NEGBINOMIAL, ZIP (ZERO INFLATED POISSON), ZINB (ZERO INFLATED NEGATIVE BINOMIAL).
...
12) whichlib: name of the SAS library where to store the output data sets generated by "model_ranfor_base" (by default=same inidcating that the same library as the model data should be used).
*/
...
%if (%sysfunc(compare("POISSON","&_distribution_")) eq 0) or 
	...
	proc plm source=&_filename_;
		score data=_set__VL&_seed_ out=_validationscore_ (keep=&id_variables &target_variable predicted) / ilink;
	run;
...
%if (%sysfunc(compare("ZIP","&_distribution_")) eq 0) or 
	...
	proc countreg restore=templib.&model_identifier. data=_set__VL&_seed_;
		score out=work._validationscore_  (keep=&id_variables &target_variable predicted pzero P_0)  mean=predicted probcount(0) probzero=pzero;
	run;
%end;
/* score validation data set -- END */

/* Accuracy Evaluation */
%accuracy_interval(work._validationscore_,&library..&model_identifier._accval,&target_variable);
...
proc sql; drop table coefficients,_set__VL&_seed_,_validationscore_,&_final_train_; quit;
