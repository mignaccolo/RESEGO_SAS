%macro bootstrap_sampling (_infile_data_,list_var_file,_output_base_filename_,_seed_,_frac_=0.7,_nrep_=1,_mylib_=work)/ 
...
In Bootstrap sampling, records can be select more than once: the variable NumberHits in TRAIN indicates the frequency 
with which a record was selected .
*/ 
...
5) _frac_: fraction of the number of records in the input SAS data set which constitute the TRAIN data set
...
%put Executing: bootstrap_sampling;
...
data &_mylib_..&_output_base_filename_._TR&_seed_. (drop=Selected &drop_replicate) &_mylib_..&_output_base_filename_._VL&_seed_. (drop=Selected &drop_replicate NumberHits);
	set work.outboot;
